...
class WindowDataset(Dataset):
    def __init__(self, series_df: pd.DataFrame, events_df: pd.DataFrame, config: dict, series_ids: set[str], split: str = "train"):
        ...
        self.lazy_cache_size = max(1, int(config.get("training", {}).get("lazy_cache_size", 2)))
    ...
    def _get_lazy_series(self, sid: str):
        cached = self.feature_cache.get(sid)
        if cached is not None:
            self.feature_cache.move_to_end(sid)
            return cached
        g = self.series_cache[sid]
        x, names = build_features(g, **self.config.get("features", {}))
        if self.feature_names is None:
            self.feature_names = names
        targets = make_training_targets(g, self.events_df, self.config)
        cached = (g["step"].to_numpy(), x, targets)
        self.feature_cache[sid] = cached
        if len(self.feature_cache) > self.lazy_cache_size:
    ...
    def __getitem__(self, idx):
        if self.lazy_windows:
            ...
            steps, x, targets = self._get_lazy_series(sid)
            ...
            out = {
                ...
                "steps": torch.from_numpy(
                    np.pad(steps[start:end], (0, len(w["mask"]) - valid_len), constant_values=-1).astype("int64")
                ),
            }
